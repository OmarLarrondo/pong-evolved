...
/**
 * Vista del panel del modo constructor/editor de mapas para el juego Pong Evolved.
 * Gestiona la carga y configuracion de la interfaz de creacion y edicion de niveles
 * personalizados con estetica retro.
 * Implementa el componente Vista del patron MVC.
 */
public class VistaEditor {

    private static final String RUTA_FXML = "fxml/editor.fxml";
    private static final String RUTA_CSS = "css/estilo-retro.css";
    private static final double DURACION_FADE_IN_MS = 500.0;

    private final ControladorEditor controlador;
    private final Scene escena;

    /**
     * Constructor que inicializa la vista del editor de mapas.
     *
     * @param controlador Controlador asociado a esta vista
     */
    public VistaEditor(ControladorEditor controlador) {
        this.controlador = controlador;
        this.escena = crearEscena();
    }

    /**
     * Crea y configura la escena del editor con su contenido FXML y estilos CSS.
     *
     * @return Scene configurada con el panel del editor
     */
    private Scene crearEscena() {
        return cargarContenido()
                .map(raiz -> construirEscena(raiz))
                .orElseThrow(() -> new RuntimeException(
                        "No se pudo cargar el contenido del editor desde " + RUTA_FXML
                ));
    }
    ...
    /**
     * Obtiene la escena del panel del editor.
     *
     * @return Scene configurada
     */
     ...
     *
     * @return ControladorEditor asociado
     */
    public ControladorEditor obtenerControlador() {
        return controlador;
    }

    /**
     * Refresca la vista reiniciando el estado del editor.
     */
